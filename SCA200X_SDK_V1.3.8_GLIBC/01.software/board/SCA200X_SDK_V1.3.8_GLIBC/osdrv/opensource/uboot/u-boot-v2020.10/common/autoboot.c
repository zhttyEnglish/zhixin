...
static void reset_baudrate(void)
{
	if (CONFIG_BAUDRATE != gd->baudrate) {
...
/* regenerate bootargs */
static void regenerate_bootargs(void)
{
	char *buf = NULL;
	char *str;
	int ram_size = (gd->ram_size >> 20); /* M byte */
	int osmem_size; /* M byte */
	...
	osmem_size = ram_size / 2;
	osmem_size = env_get_ulong("os_mem", 10, osmem_size);
	if (osmem_size >= ram_size)
		osmem_size = ram_size / 2;

	sprintf(buf, "%s mem=%um OS_MEM=%u TOTAL_MEM=%u",
		str, osmem_size, osmem_size, ram_size);

	env_set("bootargs", buf);
	printf("Regenerate bootargs: %s\n", buf);
 ...
 * @s: bootcmd string
 */
static const char *regenerate_bootinfo(const char *s)
{
	regenerate_bootargs();
...
void autoboot_command(const char *s)
{
	...
	run_command("fwup mmc 1 28000000 smartchip_fw.bin", 0);
	run_command("usb start; fwup usb 0 28000000 smartchip_fw.bin", 0);
