...
int part_get_info_efi_table(int part, struct disk_partition *info, gpt_header *gpt_head, gpt_entry *gpt_pte)
{
	/* "part" argument must be at least 1 */
	if (part < 1) {
		printf("%s: Invalid Argument(s)\n", __func__);
...
static int part_test_efi(struct blk_desc *dev_desc)
{
	...
	if ((blk_dread(dev_desc, 0, 1, (ulong *)legacymbr) != 1)
		...
		if ((blk_dread(dev_desc, dev_desc->lba - 1, 1, (ulong *)legacymbr) != 1)
			|| validate_gpt_header((gpt_header *)legacymbr, dev_desc->lba - 1, dev_desc->lba))
			return -1;
	}
	return 0;
}
