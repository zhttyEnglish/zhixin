...
#include <asm/gpio.h>
#endif
#if defined(CONFIG_USB_FUNCTION_SMARTCHIP_USB)
#include <asm/arch/boot.h>
#include <asm/arch/image.h>
#include <asm/arch/smartchip_usb.h>
#include <button.h>
#endif
#include <asm/arch/image.h>
...
static int initr_usb_upgrade(void)
{
	...
	{
		printf("uboot detect usb upgrade!\n");
		smartchip_hid_upgrade(0);
	}

	if(BOOTROM_DEVICE_UART == boot_info)
	{
		printf("uboot detect uart upgrade!\n");
		//smartchip_hid_upgrade(0);
	}

#if 0 //defined(CONFIG_BUTTON_GPIO)
	struct udevice *dev;
	int ret;
	...
	if(0 == ret)
	{
		...
		{
			printf("uboot button: detect usb upgrade!\n");
			smartchip_hid_upgrade(0);
		}
	}
#endif
...
#ifdef CONFIG_SCA200V100_BD2_DEV
static int initr_board2_fix(void)
{
	/*
	 * @what: board2 v1.0, disable da9062 ldo3
	 *
	 * @why:
	 *   sca200v100 board2 v1.0: sd0 vq from da9062 ldo3 or io board
	 *   sca200v100 board2 v2.0: sd0 vq from io board
	 *   fix: both from io board
	 */
	run_command("i2c dev 0", 0);
	run_command("i2c mw 0x58 0x28 0x0", 0);
	return 0;
}
#endif
