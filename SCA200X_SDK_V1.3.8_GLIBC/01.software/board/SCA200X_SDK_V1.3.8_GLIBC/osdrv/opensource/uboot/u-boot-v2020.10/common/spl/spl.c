			  ...
			  struct spl_image_loader *loader)
{
	...
	if (!ret && spl_image->dcrc_length) {
		...
		if (dcrc != spl_image->dcrc) {
			...
			puts("SPL: Image data CRC check OK\n");
		}
	}
#endif
	return ret;
...
#endif
extern void spl_upgrage(void);

/* sca200 usb boot: no use board_init_r.
 *   bootrom init dram.
 *   goto u-boot at arch/arm/mach-smartchip/spl.c::board_init_f()
 *
 * sca200 uart boot: use board_init_r to init dram.
 */
...
void preloader_console_init(void)
{
	...
	unsigned int boot_info = get_spl_boot_dev();
	if(BOOTROM_DEVICE_UART == boot_info)
		gd->baudrate = get_spl_boot_brate();
	else
