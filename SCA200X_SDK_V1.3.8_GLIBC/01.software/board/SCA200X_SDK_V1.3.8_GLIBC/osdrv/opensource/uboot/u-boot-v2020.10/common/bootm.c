...
#if	CONFIGSC_SEC_IMGHD
static void do_bootm_ch_addr(char *addrstr)
{
	unsigned long addr = simple_strtoul(addrstr, NULL, 16);

	struct smartx_image_header *sc_header = (struct smartx_image_header *)addr;
	...
	data = signature_buffer + sc_header->sig_len;
	...
	if(secure_boot_enabled()) {
		//printf("%p %p %p %p %u %u %u %u\n", sc_header, hash_buffer, signature_buffer, data,
		//	sizeof(*sc_header), sc_header->hash_size, sc_header->sig_len, sc_header->img_len);

		if(0 != spl_verify_sw(
			hash_buffer, sc_header->hash_size,
			signature_buffer, sc_header->sig_len,
			data, sc_header->img_len)) {
	...
	addr += (sizeof(*sc_header) + sc_header->hash_size + sc_header->sig_len);
