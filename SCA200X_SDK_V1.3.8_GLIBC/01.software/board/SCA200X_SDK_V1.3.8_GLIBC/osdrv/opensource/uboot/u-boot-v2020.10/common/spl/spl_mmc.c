			   ...
			   ulong sector, struct image_header *header)
{
	...
	struct smartx_image_header *sc_header = (struct smartx_image_header *)spl_get_load_buffer(-0x1000, bd->blksz);
	unsigned long offset = 0;
	if(0x4152544f == sc_header->magic)
	{
		offset = sizeof(*sc_header) + sc_header->hash_size + sc_header->sig_len;
	}
			      ...
			      struct mmc *mmc, unsigned long sector)
{
	...
	struct blk_desc *bd = mmc_get_blk_desc(mmc);
	int ret = 0;
	struct smartx_image_header *sc_header;
	unsigned char * hash_buffer = NULL;
	unsigned char * signature_buffer = NULL;

	sc_header = (struct smartx_image_header *)spl_get_load_buffer(-0x1000, bd->blksz);

	/* read image header to find the image size & load address */
	count = blk_dread(bd, sector, 1, sc_header);
	...
	debug("sc_header read 0x%x %u %u\n", sc_header->magic, sc_header->sig_len, sc_header->hash_size);
	if(0x4152544f != sc_header->magic)
	{
		printf("%s[%d]: magic error!\n", __FUNCTION__, __LINE__);
		hang();
	}
	sc_header = (struct smartx_image_header *)spl_get_load_buffer(-(sizeof(*header) + sizeof(*sc_header) + sc_header->hash_size + sc_header->sig_len), bd->blksz);
	...
	hash_buffer = ((unsigned char *)sc_header) + sizeof(*sc_header);
	signature_buffer = hash_buffer + sc_header->hash_size;
	header = (struct image_header *)(signature_buffer + sc_header->sig_len);
	...
	{
		if(0 != spl_verify((unsigned char *)hash_buffer, sc_header->hash_size, 
				(unsigned char *)signature_buffer, sc_header->sig_len, 
				(unsigned char *)header, sc_header->img_len))
		{
