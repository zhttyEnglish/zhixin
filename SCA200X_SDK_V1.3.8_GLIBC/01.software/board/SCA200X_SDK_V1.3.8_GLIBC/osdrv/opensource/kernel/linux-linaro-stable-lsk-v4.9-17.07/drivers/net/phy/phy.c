...
static const char *phy_state_to_str(enum phy_state st)
{
	...
	PHY_STATE_STR(NOLINK2)
	PHY_STATE_STR(HWRESET)
...
static void phy_hw_reset(struct phy_device *phydev)
{
	if (phydev->mdio.bus && phydev->mdio.bus->phy_hw_reset) {
		phydev->mdio.bus->phy_hw_reset(phydev->mdio.bus);
		phy_init_hw(phydev);
	}
}
...
void phy_state_machine(struct work_struct *work)
{
	...
	case PHY_NOLINK:
		...
		if (phydev->link) {
			if (phydev->is_hw_reset) {
				phy_hw_reset(phydev);
				phydev->state = PHY_HWRESET;
				phydev->reset_timeout = PHY_RESET_TIMEOUT;
				break;
			}
			phydev->state = PHY_NOLINK2;
		}
		break;
	...
	case PHY_HWRESET:
		...
		if (phydev->link) {
			phydev->state = PHY_NOLINK2;
		} else if (0 == phydev->reset_timeout--) {
			phydev->state = PHY_NOLINK;
		}
		break;
	case PHY_FORCING:
