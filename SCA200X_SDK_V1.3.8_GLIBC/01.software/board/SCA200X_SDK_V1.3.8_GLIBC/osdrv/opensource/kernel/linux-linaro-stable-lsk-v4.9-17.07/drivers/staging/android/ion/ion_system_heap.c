				      ...
				      unsigned long order)
{
	...
	if (cached)
		ion_pages_sync_for_device(buffer->dev->dev.this_device, page, PAGE_SIZE << order,
					  DMA_BIDIRECTIONAL);
...
struct ion_heap *ion_system_heap_create(struct device *dev, struct ion_platform_heap *unused)
{
					   ...
					   unsigned long flags)
{
	...
	ion_pages_sync_for_device(buffer->dev->dev.this_device, page, len, DMA_BIDIRECTIONAL);
...
struct ion_heap *ion_system_contig_heap_create(struct device *dev, struct ion_platform_heap *unused)
{
	struct ion_heap *heap;
