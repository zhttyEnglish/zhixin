...
struct f_uvc_opts {
	...
	char						*control_interface_name;
	char						*streaming_interface_name;
	...
	struct usb_os_desc		uvc_os_desc;
	char				uvc_ext_compat_id[16];
#endif
	...
	struct usb_gadget_strings 			*uvc_function_strings[2];
	struct usb_gadget_strings 			uvc_stringtab;
	/*
	 * uvc_en_us_strings[0] for UVC_STRING_CONTROL_IDX
	 * uvc_en_us_strings[1] for UVC_STRING_STREAMING_IDX
	 * uvc_en_us_strings[2] is NULL
	 */
	struct usb_string 				uvc_en_us_strings[3];
};
