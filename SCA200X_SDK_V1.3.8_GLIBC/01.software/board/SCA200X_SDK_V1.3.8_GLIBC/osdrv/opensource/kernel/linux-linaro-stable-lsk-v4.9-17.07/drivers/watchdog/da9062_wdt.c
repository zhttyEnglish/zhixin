...
static int da9062_wdt_start(struct watchdog_device *wdd)
{
	...
	ret = regmap_update_bits(wdt->hw->regmap,
				 DA9062AA_GPIO_0_1,
				 DA9062AA_GPIO0_WEN_MASK,
				 0 << DA9062AA_GPIO0_WEN_SHIFT);

	if (ret)
		dev_err(wdt->hw->dev, "Watchdog failed disable wakeup"
			"on DA9062AA_GPIO_0_1 (err = %d)\n",
			ret);

	ret = regmap_update_bits(wdt->hw->regmap,
				 DA9062AA_GPIO_0_1,
				 DA9062AA_GPIO0_TYPE_MASK,
				 0 << DA9062AA_GPIO0_TYPE_SHIFT);

	if (ret)
		dev_err(wdt->hw->dev, "Watchdog failed set active to low"
			"on DA9062AA_GPIO_0_1 (err = %d)\n",
			ret);

	if (IS_ERR(wdt->hw->reset_gpio)) {
		ret = regmap_update_bits(wdt->hw->regmap,
					 DA9062AA_GPIO_0_1,
					 DA9062AA_GPIO0_PIN_MASK,
					 1 << DA9062AA_GPIO0_PIN_SHIFT);
	} else {
		ret = regmap_update_bits(wdt->hw->regmap,
					 DA9062AA_GPIO_0_1,
					 DA9062AA_GPIO0_PIN_MASK,
					 0 << DA9062AA_GPIO0_PIN_SHIFT);
	}

	if (ret)
		dev_err(wdt->hw->dev, "Watchdog failed set pin"
			"onDA9062AA_GPIO_0_1 (err = %d)\n",
			ret);
...
static int da9062_wdt_set_shutdown_register(struct da9062_watchdog *wdt)
{
	struct da9062 *chip = wdt->hw;
				  ...
				  unsigned int timeout)
{
	...
	if (timeout) {
		...
		ret = gpiod_direction_output(chip->reset_gpio, 1);
		if (ret)
			dev_err(wdt->hw->dev, "Failed to set output direction (err = %d)\n", ret);
		mdelay(2);

		/* drive inactive, also active->inactive edge */
		gpiod_set_value(chip->reset_gpio, 0);
		if (ret)
			dev_err(wdt->hw->dev, "Failed to set gpio value (err = %d)\n", ret);
		mdelay(2);
		...
		if (ret)
			dev_err(wdt->hw->dev, "Failed to set gpio value (err = %d)\n", ret);
#else
		da9062_wdt_set_shutdown_register(wdt);
#endif
	}
