...
static int mmc_decode_scr(struct mmc_card *card)
{
	...
	pr_info("%s: SCR sd version %d\n",
		mmc_hostname(card->host),
		scr->sda_spec3 ? 3 : scr->sda_vsn);
	return 0;
}
...
static int mmc_read_switch(struct mmc_card *card)
{
	...
	pr_info("%s: card speed 0x%02x\n",
		mmc_hostname(card->host), status[13]);
...
MMC_DEV_ATTR(scr_p, "spec2:%d spec3:%d busw:%d\n",
	card->scr.sda_vsn,
	card->scr.sda_spec3,
	card->scr.bus_widths);
MMC_DEV_ATTR(ssr,
