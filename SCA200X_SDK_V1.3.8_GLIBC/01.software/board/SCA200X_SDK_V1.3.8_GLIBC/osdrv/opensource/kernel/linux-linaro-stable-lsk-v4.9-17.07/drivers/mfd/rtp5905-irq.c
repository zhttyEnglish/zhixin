...
static void rtp5901_irq_work(struct work_struct *work)
{
	struct rtp5901_mfd_chip *chip = container_of(work, struct rtp5901_mfd_chip, irq_work);
	uint64_t irqs = 0;
...
static irqreturn_t rtp5901_irq(int irq, void *data)
{
	struct rtp5901_mfd_chip *chip = data;
...
int rtp5901_irq_init(struct rtp5901_mfd_chip *chip, int irq)
{
	int ret;

	printk(KERN_DEBUG "Init rtp5901 irq, irq = %d\n", irq);
	...
	chip->chip_irq = irq;
	INIT_WORK(&chip->irq_work, rtp5901_irq_work);
	BLOCKING_INIT_NOTIFIER_HEAD(&chip->notifier_list);

	ret = request_irq(irq, rtp5901_irq, IRQF_DISABLED, "rtp5901_mfd", chip);
	if (ret != 0) {
