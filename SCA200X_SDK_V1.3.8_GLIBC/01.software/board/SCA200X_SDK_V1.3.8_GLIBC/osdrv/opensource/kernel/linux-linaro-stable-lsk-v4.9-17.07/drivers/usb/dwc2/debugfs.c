...
static int connect_state_show(struct seq_file *seq, void *v)
{
	struct dwc2_hsotg *hsotg = seq->private;
...
static int connect_state_open(struct inode *inode, struct file *file)
{
	return single_open(file, connect_state_show, inode->i_private);
}

static const struct file_operations connect_state_fops = {
	.owner		= THIS_MODULE,
	.open		= connect_state_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static void dwc2_hsotg_create_debug(struct dwc2_hsotg *hsotg)
{
	...
	file = debugfs_create_file("connect_state", 0444, root, hsotg, &connect_state_fops);
	if (IS_ERR(file))
		dev_err(hsotg->dev, "%s: failed to create connect_state\n", __func__);
