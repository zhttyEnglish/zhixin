			 ...
			 const struct v4l2_event_subscription *sub)
{
	...
	if (sub->type < UVC_EVENT_FIRST || sub->type > UVC_EVENT_LAST)
		return -EINVAL;
	//printk(KERN_INFO "uvc_v4l2_subscribe_event %d\n", sub->type);
	ret = v4l2_event_subscribe(fh, sub, 2, NULL);
	if(ret >= 0) {
		struct video_device *vdev = fh->vdev;
		if(vdev) {
			struct uvc_device *uvc = video_get_drvdata(vdev);
			if(0 == uvc->event_subscribed[sub->type - UVC_EVENT_FIRST]) {
				uvc->event_subscribed[sub->type - UVC_EVENT_FIRST] = 1;
				if(UVC_EVENT_SETUP == sub->type)
...
{
	struct video_device *vdev = fh->vdev;
	int ret = v4l2_event_unsubscribe(fh, sub);

	if(ret >= 0 && vdev) {
		struct uvc_device *uvc = video_get_drvdata(vdev);

		if (sub->type >= UVC_EVENT_FIRST && sub->type <= UVC_EVENT_LAST)
			uvc->event_subscribed[sub->type - UVC_EVENT_FIRST] = 0;
		else if(sub->type == V4L2_EVENT_ALL)
