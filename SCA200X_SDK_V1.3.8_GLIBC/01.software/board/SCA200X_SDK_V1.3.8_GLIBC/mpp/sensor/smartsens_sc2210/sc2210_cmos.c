/**
 * @file     sc2210_cmos.c
 * @brief    SMART SEMS SC2210 SENSOR控制回调接口的实现
 * @version  1.0.0
 * @since    1.0.0
...
#if !defined(__SC2210_CMOS_C_)
#define __SC2210_CMOS_C_
 ...
 * Global variables                                                         *
 ****************************************************************************/
ISP_SNS_STATE_S *g_pastSc2210[ISP_MAX_PIPE_NUM] = {SC_NULL};

#define SENSOR_GET_CTX(dev, pstCtx) (pstCtx = g_pastSc2210[dev])
#define SENSOR_SET_CTX(dev, pstCtx) (g_pastSc2210[dev] = pstCtx)
#define SENSOR_RESET_CTX(dev)       (g_pastSc2210[dev] = SC_NULL)
...
{
    [0] = {
              .acLinearBinName = "/local/tuning/sc2210/sc2210_tuning.bin",
              .acWdrBinName    = "/local/tuning/sc2210/sc2210_tuning_hdr.bin"
          },
...
extern int  sc2210_init(VI_PIPE ViPipe);
extern void sc2210_exit(VI_PIPE ViPipe);
extern void sc2210_standby(VI_PIPE ViPipe);
extern void sc2210_restart(VI_PIPE ViPipe);
extern void sc2210_default_reg_init(VI_PIPE ViPipe);
extern int  sc2210_write_register(VI_PIPE ViPipe, int addr, int data);
extern int  sc2210_read_register(VI_PIPE ViPipe, int addr);
extern int  sc2210_flip_off_mirror_off(VI_PIPE ViPipe);
extern int  sc2210_flip_on_mirror_off(VI_PIPE ViPipe);
extern int  sc2210_flip_off_mirror_on(VI_PIPE ViPipe);
extern int  sc2210_flip_on_mirror_on(VI_PIPE ViPipe);
extern int  sc2210_linear_1080p30_pre_init(VI_PIPE ViPipe);
...
#define SC2210_EXPO_ADDR       (0x3e00)
#define SC2210_EXPO_S_ADDR     (0x3e04)
#define SC2210_AGAIN_ADDR      (0x3e08)
#define SC2210_DGAIN_ADDR      (0x3e06)
#define SC2210_AGAIN_S_ADDR    (0x3e12)
#define SC2210_DGAIN_S_ADDR    (0x3e10)
#define SC2210_FRM_LEN_ADDR    (0x320e)
#define SC2210_EXPO_MAX_S_ADDR (0x3e23)
...
static SC_S32 cmos_power_on(VI_PIPE ViPipe, dev_power_attr_t *p_dev_power_attr)
{
    ...
    sc2210_linear_1080p30_pre_init(ViPipe);
...
static SC_U32 again_table[183] =
{
     ...
     3201, 3264, 3328, 3390, 3457, 3485, 3595, 3705, 3813, 3924,
     4034, 4142, 4249, 4358, 4469, 4579, 4685, 4795, 4906, 5015,
     5120, 5233, 5343, 5448, 5556, 5666, 5778, 5885, 5995, 6106,
     6212, 6321, 6431, 6535, 6649, 6757, 6867, 6979, 7191, 7409,
     7626, 7848, 8068, 8285, 8497, 8715, 8939, 9157, 9371, 9589,
     9812, 10029, 10240, 10466, 10685, 10896, 11112, 11331, 11555, 11770,
    11989, 12212, 12425, 12641, 12861, 13085, 13298, 13514, 13734, 13957,
    14381, 14818, 15251, 15696, 16136, 16569, 16994, 17430, 17877, 18314,
    18741, 19177, 19624, 20058, 20502, 20931, 21369, 21792, 22222, 22662,
    23110, 23539, 23977, 24423, 24848, 25281, 25721, 26169, 26594, 27027,
    27466, 27912, 28760, 29634, 30535, 31390, 32270, 33135, 33985, 34857,
    35751, 36626, 37480, 38353, 39246, 40114, 41001, 41860, 42736, 43581,
    44493, 45321, 46216, 47076, 47951, 48842, 49693, 50559, 51439, 52335,
    53186, 54050, 54928
};
...
static SC_VOID cmos_again_calc_table(VI_PIPE ViPipe, SC_U32 *pu32AgainLin, SC_U32 *pu32AgainDb)
{
    ...
    if(*pu32AgainLin >= again_table[182])
    {
        *pu32AgainLin = again_table[182];
        *pu32AgainDb  = 182;
        return;
    }
...
static SC_VOID cmos_gains_update(VI_PIPE ViPipe, SC_U32 u32Again, SC_U32 u32Dgain)
{
     ...
     * [ 32.. 54]                0x07          [0x40..0x6C]                         2.0X
     * [ 55.. 86]                0x23          [0x40..0x7F]                       3.406X
     * [ 87..118]                0x27          [0x40..0x7F]                       6.812X
     * [119..150]                0x2F          [0x40..0x7F]                      13.624X
     * [151..182]                0x3F          [0x40..0x7F]                      27.248X..54.07X
     */
    //printf("sns pp=%d, ag=%d, dg=%d\n", ViPipe, u32Again, u32Dgain);
    if(u32Again < 55)
    {
        /* Analog fine gain < 3.406X */
        if(u32Again < 32)
        {
    ...
    else
    {
        ...
        {
            if(u32Again < (32 * aCoarseGain + 55))
            {
                break;
            }
        }

        aFineGain = 2 * (u32Again - 55 - 32 * (aCoarseGain - 1));
    }
    ...
    u8Reg0x3e08 = (u32Again >= 55) ? (u8Reg0x3e08 | 0x20) : (u8Reg0x3e08 & 0x1F);

    /* Analog fine gain register: from 0x40 to 0x7F(special value is from 0x40 to 0x6C), step is 2. */
...
static SC_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, SC_BOOL bEnable)
{
    ...
    if(bEnable) /* setup for ISP pixel calibration mode */
    {
        ...
        sc2210_write_register(ViPipe, 0x320e, (u32FullLines_5Fps & 0xFF00) >> 8);
        sc2210_write_register(ViPipe, 0x320f, u32FullLines_5Fps & 0xFF);
        sc2210_write_register(ViPipe, 0x3e01, (u32MaxIntTime_5Fps & 0x0FF0) >> 4);
        sc2210_write_register (ViPipe, 0x3e02, (u32MaxIntTime_5Fps  & 0xF)<<4);
    }
    else /* setup for ISP 'normal mode' */
    {
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > SENSOR_FULL_LINES_MAX) ? SENSOR_FULL_LINES_MAX : pstSnsState->u32FLStd;
        sc2210_write_register(ViPipe, 0x320e, (pstSnsState->u32FLStd & 0xFF00) >> 8);
        sc2210_write_register(ViPipe, 0x320f, pstSnsState->u32FLStd & 0xFF);
...
static SC_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if((SC_FALSE == pstSnsState->bSyncInit) || (SC_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr    = SC2210_EXPO_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr    = SC2210_EXPO_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr    = SC2210_EXPO_ADDR + 2;

        /* Linear mode gain registers */
        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr    = SC2210_DGAIN_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr    = SC2210_DGAIN_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr    = SC2210_AGAIN_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr    = SC2210_AGAIN_ADDR + 1;

        /* Frame length registers */
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr    = SC2210_FRM_LEN_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr    = SC2210_FRM_LEN_ADDR + 1;

        if(WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
        {
            /* DOL 2T1 mode exposure registers */
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum  = 0;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr     = SC2210_EXPO_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr    = SC2210_EXPO_S_ADDR + 1;

            /* DOL 2T1 mode gain registers */
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr    = SC2210_DGAIN_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr    = SC2210_DGAIN_S_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr    = SC2210_AGAIN_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr    = SC2210_AGAIN_S_ADDR + 1;

            /* DOL 2T1 mode max short exposure registers */
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr    = SC2210_EXPO_MAX_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[16].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[16].u32RegAddr    = SC2210_EXPO_MAX_S_ADDR + 1;
        }
...
static SC_VOID sc2210_global_init(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pstSnsState = SC_NULL;
...
static SC_S32 cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    ...
    switch(mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = sc2210_flip_off_mirror_off(ViPipe);
            break;
        }

        case ISP_SNS_MIRROR:
        {
            ret = sc2210_flip_off_mirror_on(ViPipe);
            break;
        }

        case ISP_SNS_FLIP:
        {
            ret = sc2210_flip_on_mirror_off(ViPipe);
            break;
        }

        case ISP_SNS_MIRROR_FLIP:
        {
            ret = sc2210_flip_on_mirror_on(ViPipe);
            break;
        }
...
static SC_S32 cmos_sc2210_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    SC_U8  u8CtlCode = 0;
    SC_S32 ret       = 0;
    SC_S32 mode      = 0;
...
static SC_S32 cmos_init_sc2210_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_global_init = sc2210_global_init;
    ...
    pstSensorExpFunc->pfn_cmos_sns_ctl       = cmos_sc2210_ctl;
 ...
 * callback structure                                                       *
 ****************************************************************************/
static SC_S32 sc2210_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static SC_S32 sc2210_ctx_init(VI_PIPE ViPipe)
{
...
static SC_VOID sc2210_ctx_exit(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pastSnsStateCtx = SC_NULL;
...
static SC_S32 sc2210_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret  = cmos_init_sc2210_exp_function(&stIspRegister.stSnsExp);
...
static SC_S32 sc2210_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    SC_S32 s32Ret = SC_FAILURE;
    ...
    s32Ret = SC_MPI_ISP_SensorUnRegCallBack(ViPipe, SC2210_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SC_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC2210_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SC_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC2210_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    sc2210_ctx_exit(ViPipe);

    return SC_SUCCESS;
}

static SC_S32 sc2210_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
...
{
    .pfnRegisterCallback    = sc2210_register_callback,
    .pfnUnRegisterCallback  = sc2210_unregister_callback,
