/**
 * @file     sc2310_cmos.c
 * @brief    SMART SEMS SC2310 SENSOR控制回调接口的实现
 * @version  1.0.0
 * @since    1.0.0
...
#if !defined(__SC2310_CMOS_C_)
#define __SC2310_CMOS_C_
...
#define SC2310_ID 2310


/****************************************************************************
 * Global variables                                                         *
 ****************************************************************************/
ISP_SNS_STATE_S *g_pastSc2310[ISP_MAX_PIPE_NUM] = {SC_NULL};

#define SENSOR_GET_CTX(dev, pstCtx) (pstCtx = g_pastSc2310[dev])
#define SENSOR_SET_CTX(dev, pstCtx) (g_pastSc2310[dev] = pstCtx)
#define SENSOR_RESET_CTX(dev)       (g_pastSc2310[dev] = SC_NULL)

ISP_SNS_COMMBUS_U g_aunSc2310BusInfo[ISP_MAX_PIPE_NUM] =
{
    [0] = { .s8I2cDev = 1},
 ...
 * extern                                                                   *
 ****************************************************************************/
extern const unsigned int sc2310_i2c_addr;
extern unsigned int sc2310_addr_byte;
extern unsigned int sc2310_data_byte;

extern int  sc2310_init(VI_PIPE ViPipe);
extern void sc2310_exit(VI_PIPE ViPipe);
extern void sc2310_standby(VI_PIPE ViPipe);
extern void sc2310_restart(VI_PIPE ViPipe);
extern void sc2310_default_reg_init(VI_PIPE ViPipe);
extern int  sc2310_write_register(VI_PIPE ViPipe, int addr, int data);
extern int  sc2310_read_register(VI_PIPE ViPipe, int addr);
extern int  sc2310_flip_off_mirror_off(VI_PIPE ViPipe);
extern int  sc2310_flip_on_mirror_off(VI_PIPE ViPipe);
extern int  sc2310_flip_off_mirror_on(VI_PIPE ViPipe);
extern int  sc2310_flip_on_mirror_on(VI_PIPE ViPipe);
...
#define SC2310_EXPO_ADDR       (0x3e00)
#define SC2310_EXPO_S_ADDR     (0x3e04)
#define SC2310_AGAIN_ADDR      (0x3e08)
#define SC2310_DGAIN_ADDR      (0x3e06)
#define SC2310_AGAIN_S_ADDR    (0x3e12)
#define SC2310_DGAIN_S_ADDR    (0x3e10)
#define SC2310_FRM_LEN_ADDR    (0x320e)
#define SC2310_EXPO_MAX_S_ADDR (0x3e23)

#define SENSOR_FULL_LINES_MAX          (0x3FFFF)
#define SENSOR_FULL_LINES_MAX_2TO1_WDR (0x3FFFF) /* considering the YOUT_SIZE and bad frame */

#define SENSOR_INCREASE_LINES       (0) /* make real fps less than stand fps because NVR require */
#define SENSOR_VMAX_1080P30_LINEAR  (1125 + SENSOR_INCREASE_LINES)
#define SENSOR_VMAX_1080P60TO30_WDR (2250 + SENSOR_INCREASE_LINES)
...
static SC_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch(pstSnsState->enWDRMode)
    {
        ...
        case WDR_MODE_NONE:
        {
            ...
            pstAeSnsDft->u32MaxIntTime = pstSnsState->u32FLStd * 2 - 6;
...
static SC_VOID cmos_slow_framerate_set(VI_PIPE ViPipe, SC_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if(WDR_MODE_NONE == pstSnsState->enWDRMode)
    {
        pstAeSnsDft->u32MaxIntTime = pstSnsState->au32FL[0] * 2 - 6;
    }
    else
    {
...
static SC_VOID cmos_gains_update(VI_PIPE ViPipe, SC_U32 u32Again, SC_U32 u32Dgain)
{
     ...
     * u32Again(tab_index)   coarse gain and fine gain reg_value(step is 2)
     * [  0.. 31]                0x03          [0x40..0x7F]
     * [ 32.. 43]                0x07          [0x40..0x56]
     * [ 44.. 75]                0x23          [0x40..0x7F]
     * [ 76..107]                0x27          [0x40..0x7F]
     * [108..139]                0x2F          [0x40..0x7F]
     * [140..171]                0x3F          [0x40..0x7F]
     */
     ...
     * u32Again(tab_index)   coarse gain and fine gain reg_value(step is 4)
     * [  0.. 31]                0x00          [0x80..0xFC]
     * [ 32.. 63]                0x01          [0x80..0xFC]
     * [ 64.. 95]                0x03          [0x80..0xFC]
     * [ 96..127]                0x07          [0x80..0xFC]
     * [128..159]                0x0F          [0x80..0xFC]
     */
...
static SC_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    pstDef->stSensorMode.u32SensorID  = SC2310_ID;
...
static SC_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, SC_BOOL bEnable)
{
    ...
    if(bEnable) /* setup for ISP pixel calibration mode */
    {
        ...
        sc2310_write_register(ViPipe, 0x320e, (u32FullLines_5Fps & 0xFF00) >> 8);
        sc2310_write_register(ViPipe, 0x320f, u32FullLines_5Fps & 0xFF);
        sc2310_write_register(ViPipe, 0x3e01, (u32MaxIntTime_5Fps & 0x0FF0) >> 4);
        sc2310_write_register (ViPipe, 0x3e02, (u32MaxIntTime_5Fps  & 0xF)<<4);
    }
    else /* setup for ISP 'normal mode' */
    {
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > SENSOR_FULL_LINES_MAX) ? SENSOR_FULL_LINES_MAX : pstSnsState->u32FLStd;
        sc2310_write_register(ViPipe, 0x320e, (pstSnsState->u32FLStd & 0xFF00) >> 8);
        sc2310_write_register(ViPipe, 0x320f, pstSnsState->u32FLStd & 0xFF);
...
static SC_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if((SC_FALSE == pstSnsState->bSyncInit) || (SC_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev   = g_aunSc2310BusInfo[ViPipe].s8I2cDev;
        ...
        for(i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr      = sc2310_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = sc2310_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = sc2310_data_byte;
        }

        /* Linear mode exposure registers, make exposure and gain effective at the same time. */
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr    = SC2310_EXPO_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr    = SC2310_EXPO_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr    = SC2310_EXPO_ADDR + 2;

        /* Linear mode gain registers */
        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr    = SC2310_DGAIN_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr    = SC2310_DGAIN_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr    = SC2310_AGAIN_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr    = SC2310_AGAIN_ADDR + 1;

        /* Frame length registers */
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr    = SC2310_FRM_LEN_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr    = SC2310_FRM_LEN_ADDR + 1;

        if(WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
        {
            /* DOL 2T1 mode exposure registers */
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum  = 0;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr     = SC2310_EXPO_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr    = SC2310_EXPO_S_ADDR + 1;

            /* DOL 2T1 mode gain registers */
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr    = SC2310_DGAIN_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr    = SC2310_DGAIN_S_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr    = SC2310_AGAIN_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr    = SC2310_AGAIN_S_ADDR + 1;

            /* DOL 2T1 mode max short exposure registers */
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr    = SC2310_EXPO_MAX_S_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[16].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[16].u32RegAddr    = SC2310_EXPO_MAX_S_ADDR + 1;
        }
    ...
    sc2310_default_reg_init(ViPipe);
...
static SC_VOID sc2310_global_init(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pstSnsState = SC_NULL;
    ...
    pstSnsState->bSyncInit = SC_FALSE;
    pstSnsState->u8ImgMode = SENSOR_1080P_30FPS_2T1_WDR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_2To1_LINE;
    pstSnsState->u32FLStd  = SENSOR_VMAX_1080P60TO30_WDR;
    pstSnsState->au32FL[0] = SENSOR_VMAX_1080P60TO30_WDR;
    pstSnsState->au32FL[1] = SENSOR_VMAX_1080P60TO30_WDR;
    if(WDR_MODE_NONE == pstSnsState->enWDRMode)
    {
        ...
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = 0x04;   //vts
...
static SC_S32 cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    ...
    switch(mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = sc2310_flip_off_mirror_off(ViPipe);
            break;
        }

        case ISP_SNS_MIRROR:
        {
            ret = sc2310_flip_off_mirror_on(ViPipe);
            break;
        }

        case ISP_SNS_FLIP:
        {
            ret = sc2310_flip_on_mirror_off(ViPipe);
            break;
        }

        case ISP_SNS_MIRROR_FLIP:
        {
            ret = sc2310_flip_on_mirror_on(ViPipe);
            break;
        }
...
static SC_S32 cmos_sc2310_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    SC_U8  u8CtlCode = 0;
    SC_S32 ret       = 0;
    SC_S32 mode      = 0;
...
static SC_S32 cmos_init_sc2310_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    CMOS_CHECK_POINTER(pstSensorExpFunc);

    memset(pstSensorExpFunc, 0, sizeof(ISP_SENSOR_EXP_FUNC_S));
    pstSensorExpFunc->pfn_cmos_sensor_init        = sc2310_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit        = sc2310_exit;
    pstSensorExpFunc->pfn_cmos_sensor_global_init = sc2310_global_init;
    ...
    pstSensorExpFunc->pfn_cmos_sns_ctl       = cmos_sc2310_ctl;
 ...
 * callback structure                                                       *
 ****************************************************************************/
static SC_S32 sc2310_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunSc2310BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;

    return SC_SUCCESS;
}

static SC_S32 sc2310_ctx_init(VI_PIPE ViPipe)
{
...
static SC_VOID sc2310_ctx_exit(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pastSnsStateCtx = SC_NULL;
...
static SC_S32 sc2310_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = sc2310_ctx_init(ViPipe);
    if(SC_SUCCESS != s32Ret)
    {
        return SC_FAILURE;
    }

    stSnsAttrInfo.eSensorId = SC2310_ID;

    s32Ret  = cmos_init_sc2310_exp_function(&stIspRegister.stSnsExp);
...
static SC_S32 sc2310_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    SC_S32 s32Ret = SC_FAILURE;
    ...
    s32Ret = SC_MPI_ISP_SensorUnRegCallBack(ViPipe, SC2310_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SC_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC2310_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SC_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC2310_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    sc2310_ctx_exit(ViPipe);

    return SC_SUCCESS;
}

static SC_S32 sc2310_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
...
ISP_SNS_OBJ_S stSnsSc2310Obj =
{
    .pfnRegisterCallback    = sc2310_register_callback,
    .pfnUnRegisterCallback  = sc2310_unregister_callback,
    .pfnStandby             = sc2310_standby,
    .pfnRestart             = sc2310_restart,
    .pfnMirrorFlip          = SC_NULL,
    .pfnWriteReg            = sc2310_write_register,
    .pfnReadReg             = sc2310_read_register,
    .pfnSetBusInfo          = sc2310_set_bus_info,
    .pfnSetInit             = sc2310_set_init
};
