...
#if !defined(__IMX415_CMOS_C_)
#define __IMX415_CMOS_C_
...
ISP_SNS_STATE_S *g_pastImx415[ISP_MAX_PIPE_NUM] = {SC_NULL};

#define SENSOR_GET_CTX(dev, pstCtx) (pstCtx = g_pastImx415[dev])
#define SENSOR_SET_CTX(dev, pstCtx) (g_pastImx415[dev] = pstCtx)
#define SENSOR_RESET_CTX(dev)       (g_pastImx415[dev] = SC_NULL)
...
static SENSOR_STATE_S g_astimx415State[ISP_MAX_PIPE_NUM] = {{0}};

static SC_TUNING_BIN_FILE_S g_acTuningBinName[ISP_MAX_PIPE_NUM] =
{
    [0] = {
              .acLinearBinName = "/local/tuning/imx415/imx415_tuning.bin",
              .acWdrBinName    = "/local/tuning/imx415/imx415_tuning_hdr.bin"
          },
    [1] = {
              .acLinearBinName = "/local/tuning/imx415/imx415_tuning.bin",
              .acWdrBinName    = "/local/tuning/imx415/imx415_tuning_hdr.bin"
...
extern int  imx415_init(VI_PIPE ViPipe);
...
extern int  imx415_flip_off_mirror_off(VI_PIPE ViPipe);
extern int  imx415_flip_on_mirror_off(VI_PIPE ViPipe);
extern int  imx415_flip_off_mirror_on(VI_PIPE ViPipe);
extern int  imx415_flip_on_mirror_on(VI_PIPE ViPipe);
...
#define SENSOR_FULL_LINES_MAX           (0x0FFFFF)
#define SENSOR_FULL_LINES_MAX_2TO1_WDR  (0x8AA) /* considering the YOUT_SIZE and bad frame */
#define SENSOR_FULL_LINES_MAX_3TO1_WDR  (0x7FC)

#define SENSOR_INCREASE_LINES    (1)  /* make real fps less than stand fps because NVR require */
#define SENSOR_VMAX_4KP30_LINEAR (2250 + SENSOR_INCREASE_LINES)
#define SENSOR_VMAX_4KP60_LINEAR 2250

#define SENSOR_4K_30FPS_LINEAR_MODE (1)
#define SENSOR_4K_60FPS_LINEAR_MODE (2)

#define SENSOR_RES_IS_4K(w, h) ((w) <= 4000 && (h) <= 3000)
...
static SC_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if(pstSnsState->u8ImgMode == SENSOR_4K_30FPS_LINEAR_MODE)
    {
        pstAeSnsDft->f32MaxFps = 30;
        pstAeSnsDft->f32Fps    = 30;
    }
    else if(pstSnsState->u8ImgMode == SENSOR_4K_60FPS_LINEAR_MODE)
    {
...
{
    ISP_SNS_STATE_S *pstSnsState = SC_NULL;
    SC_U32 u32VMAX = SENSOR_VMAX_4KP30_LINEAR;
    ...
    {
        case SENSOR_4K_30FPS_LINEAR_MODE:
        {
            if((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = SENSOR_VMAX_4KP30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
        ...
        case SENSOR_4K_60FPS_LINEAR_MODE:
        {
            if((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = SENSOR_VMAX_4KP60_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
...
static SC_VOID cmos_inttime_update(VI_PIPE ViPipe, SC_U32 u32IntTime)
{
    ...
    //static SC_U8 u8Count[ISP_MAX_PIPE_NUM] = {0};

    //static SC_U32 u32ShortIntTime[ISP_MAX_PIPE_NUM]  = {0};
    //static SC_U32 u32ShortIntTime1[ISP_MAX_PIPE_NUM] = {0};
    //static SC_U32 u32ShortIntTime2[ISP_MAX_PIPE_NUM] = {0};
    //static SC_U32 u32LongIntTime[ISP_MAX_PIPE_NUM]   = {0};

    //static SC_U32 u32RHS1[ISP_MAX_PIPE_NUM]  = {0};
    //static SC_U32 u32RHS2[ISP_MAX_PIPE_NUM]  = {0};
    //static SC_U32 u32SHS1[ISP_MAX_PIPE_NUM]  = {0};
    //static SC_U32 u32SHS2[ISP_MAX_PIPE_NUM]  = {0};

    //SC_U32 u32YOUTSIZE = 0;
...
{
    ISP_SNS_STATE_S *pstSnsState = SC_NULL;
    SC_U32 u32HCG = g_astimx415State[ViPipe].u8Hcg;
    SC_U32 u32Tmp = 0;
...
static SC_VOID cmos_get_inttime_max(VI_PIPE ViPipe, SC_U16 u16ManRatioEnable, SC_U32 *au32Ratio, SC_U32 *au32IntTimeMax,
    SC_U32 *au32IntTimeMin, SC_U32 *pu32LFMaxIntTime)
{
    SC_U32 u32IntTimeMaxTmp0 = 0;
    SC_U32 u32IntTimeMaxTmp  = 0;
    //SC_U32 u32RHS2_Max = 0;
    ...
    if(WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astimx415State[ViPipe].u32RHS1_MAX - 3)) ?
                (g_astimx415State[ViPipe].u32RHS1_MAX - 3) : u32IntTimeMaxTmp;
...
static SC_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, SC_BOOL bEnable)
{
    ...
    {
        if(SENSOR_4K_30FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            u32FullLines_5Fps = (SENSOR_VMAX_4KP30_LINEAR * 30) / 5;
        }
        else if(SENSOR_4K_60FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            // u32FullLines_5Fps = (SENSOR_VMAX_4KP60_LINEAR * 60) / 5;
            return;
        }
        else
        {
...
static SC_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, SC_U8 u8Mode)
{
    ...
    switch(u8Mode)
    {
        ...
        case WDR_MODE_2To1_LINE:
        {
            ...
            //printf("cmos_set_wdr_mode: 2to1 line wdr mode\n");
            //break;
        }
...
static SC_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if((SC_FALSE == pstSnsState->bSyncInit) || (SC_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr    = IMX415_GAIN_ADDR;
...
static SC_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if(pstSensorImageMode->f32Fps <= 30)
    {
        if(WDR_MODE_NONE == pstSnsState->enWDRMode)
        {
            if(SENSOR_RES_IS_4K(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                printf("u8SensorImageMode:%d\n", u8SensorImageMode);
                u8SensorImageMode     = SENSOR_4K_30FPS_LINEAR_MODE;
                pstSnsState->u32FLStd = SENSOR_VMAX_4KP30_LINEAR;
    ...
    else
    {
        ...
        {
            printf("u8SensorImageMode:%d\n", u8SensorImageMode);
            u8SensorImageMode     = SENSOR_4K_60FPS_LINEAR_MODE;
            pstSnsState->u32FLStd = SENSOR_VMAX_4KP60_LINEAR;
            //g_astimx415State[ViPipe].u8Hcg = 0x2;
        }
        else
        {
...
static SC_VOID imx415_global_init(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pstSnsState = SC_NULL;
    ...
    pstSnsState->bSyncInit = SC_FALSE;
    pstSnsState->u8ImgMode = SENSOR_4K_30FPS_LINEAR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd  = SENSOR_VMAX_4KP30_LINEAR;
    pstSnsState->au32FL[0] = SENSOR_VMAX_4KP30_LINEAR;
    pstSnsState->au32FL[1] = SENSOR_VMAX_4KP30_LINEAR;
...
static SC_S32 cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    ...
    switch(mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = imx415_flip_off_mirror_off(ViPipe);
            break;
        }

        case ISP_SNS_MIRROR:
        {
            ret = imx415_flip_off_mirror_on(ViPipe);
            break;
        }

        case ISP_SNS_FLIP:
        {
            ret = imx415_flip_on_mirror_off(ViPipe);
            break;
        }

        case ISP_SNS_MIRROR_FLIP:
        {
            ret = imx415_flip_on_mirror_on(ViPipe);
            break;
        }
...
static SC_S32 cmos_imx415_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    SC_U8  u8CtlCode = 0;
    SC_S32 ret       = 0;
    SC_S32 mode      = 0;
...
static SC_S32 cmos_init_imx415_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_global_init = imx415_global_init;
    ...
    pstSensorExpFunc->pfn_cmos_sns_ctl          = cmos_imx415_ctl;
 ...
 * callback structure                                                       *
 ****************************************************************************/
static SC_S32 imx415_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static SC_S32 imx415_ctx_init(VI_PIPE ViPipe)
{
...
static SC_VOID imx415_ctx_exit(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pastSnsStateCtx = SC_NULL;
...
static SC_S32 imx415_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    SC_S32 s32Ret;
    ...
    s32Ret  = cmos_init_imx415_exp_function(&stIspRegister.stSnsExp);
...
static SC_S32 imx415_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    SC_S32 s32Ret;
    ...
    s32Ret = SC_MPI_ISP_SensorUnRegCallBack(ViPipe, IMX415_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SC_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, IMX415_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SC_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, IMX415_ID);
    if(SC_SUCCESS != s32Ret)
    {
    ...
    imx415_ctx_exit(ViPipe);

    return SC_SUCCESS;
}

static SC_S32 imx415_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
...
{
    .pfnRegisterCallback    = imx415_register_callback,
    .pfnUnRegisterCallback  = imx415_unregister_callback,
