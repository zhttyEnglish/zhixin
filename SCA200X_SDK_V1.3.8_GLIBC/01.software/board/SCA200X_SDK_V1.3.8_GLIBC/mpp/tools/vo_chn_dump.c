...
******************************************************************************/
void VOU_Chn_HandleSig(SC_S32 signo)
{
    if (SIGINT == signo || SIGTERM == signo)
    {
        if (0 != g_stFrame.stVFrame.u64PhyAddr[0])
        {
            SC_MPI_VO_ReleaseChnFrame(0, g_VoChn, &g_stFrame);
...
static SC_S32 SAMPLE_MISC_VoDump(VO_DEV VoDev, SC_S32 s32Cnt)
{
    SC_S32             i, s32Ret;
    ...
    s32Ret = SC_MPI_VO_GetChnFrame(VoDev, g_VoChn, &g_stFrame, 0);
    ...
    snprintf(szYuvName, 128, "./vo_layer%d_chn%d_%ux%u_%s_%s_%d.yuv",
        VoDev,
        g_VoChn,
    ...
    SC_MPI_VO_ReleaseChnFrame(VoDev, g_VoChn, &g_stFrame);
    ...
    for (i = 0; i < s32Cnt; i++)
    {
        s32Ret = SC_MPI_VO_GetChnFrame(VoDev, g_VoChn, &g_stFrame, 20);
        if (SC_SUCCESS != s32Ret)
        {
        ...
        s32Ret = SC_MPI_VO_ReleaseChnFrame(VoDev, g_VoChn, &g_stFrame);
        memset(&g_stFrame, 0, sizeof(VIDEO_FRAME_INFO_S));
        if (SC_SUCCESS != s32Ret)
        {
