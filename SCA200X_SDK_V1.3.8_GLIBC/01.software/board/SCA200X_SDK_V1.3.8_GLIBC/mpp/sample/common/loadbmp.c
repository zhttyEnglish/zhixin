...
SC_U16 OSD_MAKECOLOR_U16(SC_U8 r, SC_U8 g, SC_U8 b, OSD_COMP_INFO compinfo)
{
    SC_U8 r1, g1, b1;
    SC_U16 pixel = 0;
    SC_U32 tmp = 15;
...
SC_S32 GetBmpInfo(const char *filename,     OSD_BITMAPFILEHEADER  *pBmpFileHeader
...
int LoadBMP(const char *filename, OSD_LOGO_T *pVideoLogo)
{
    ...
    pVideoLogo->width = (SC_U16)bmpInfo.bmiHeader.biWidth;
    pVideoLogo->height = (SC_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight :
    ...
    #endif
    /* RGB8888 or RGB1555 */
    pOrigBMPBuf = (SC_U8 *)malloc(h * stride);
    if (NULL == pOrigBMPBuf)
    {
        ...
        for (j = 0; j < w; j++)
        {
            if (Bpp == 3)
            {
                ...
                case OSD_COLOR_FMT_RGB4444:
                    ...
                    pDst = (SC_U16 *)(pRGBBuf + i * pVideoLogo->stride + j * 2);
                    r = *(pStart);
                    g = *(pStart + 1);
                    b = *(pStart + 2);
...
int LoadBMPCanvas(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    w = (SC_U16)bmpInfo.bmiHeader.biWidth;
    h = (SC_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));
