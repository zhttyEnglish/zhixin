...
int FILTER_DoTest_Task(char *p_data_base_path, epri_filter_test *p_filter_info,epri_filter_test_res *p_filter_res);
...
static int  filter_json_proc_cb(void *p_cJson)
{
    printf("filter_json_proc_cb\n");
    cJSON               *p_root_cJson = (cJSON*)p_cJson;
    cJSON               *p_response_cJson = NULL;
    epri_filter_test_res    test_json_res = {0};
    epri_filter_test       *p_test_json = NULL;
    int res = 0;
    ...
    //将json 字符串转换为 结构体
    p_test_json = (epri_filter_test*)json_to_struct_epri_filter_test(p_root_cJson);
    if(NULL != p_test_json)
    {
        //获得资源的完整路径
        ...
        res = FILTER_DoTest_Task(g_base_path, p_test_json, &test_json_res);
        /*****************************************************************************/
        ...
        test_json_res.group_id = p_test_json->group_id;

        //change struct to json.
        p_response_cJson = struct_to_json_epri_filter_test_res(&test_json_res);
...
static int issud_svp_image_load(SVP_NPU_MODEL_CONF_S model_conf,MODEL_INFO &model_info,char *name)
{
    int ret;
    char mem_name[128];
    /*load model*/
...
static int mat_to_svp(Mat &src_img,SVP_IMAGE_S& image,SVP_IMAGE_S& dst_image)
{
...
static int BboxDecoding(MODEL_INFO model_info,auto Buffer, int tensorIndex, detection_issue *detsObj,int totalClass)
{
	float x, y, w, h;
...
static void nms(detection_issue *dets, int total, int classes, float thresh)
{
    int i, j, k;
    k = total-1;
    for(i = 0; i <= k; ++i){
...
static int issuePostProcess(MODEL_INFO model_info,auto buff, std::vector<DETECTION_RESULT> & vOutput,detection_issue *dets,int totalClass)
{
	int inputTensorW = model_info.model.astSrcTensor[0].u32Width;
	int inputTensorH = model_info.model.astSrcTensor[0].u32Height;
...
static int get_issue_result(MODEL_INFO model_info,std::vector<ISSUE_RESULT>& det_rects,detection_issue *dets,int totalClass)
{
    det_rects.clear();
...
static void transform(const int &ih, const int &iw, const int &oh, const int &ow, float& xmin, float& ymin, float& xmax, float& ymax)
{
...
static int issue_svp_image_unload(MODEL_INFO& model_info)
{
...
*/
static float iou_filter(epri_issues_filter rect1, epri_issues_filter rect2) 
{
    float xA = rect1.x;
    float yA = rect1.y;
    float xB = rect1.x + rect1.w;
    float yB = rect1.y + rect1.h;
    float xC = rect2.x;
    float yC = rect2.y;
    float xD = rect2.x + rect2.w;
    float yD = rect2.y + rect2.h;
    ...
    float areaA = rect1.w * rect1.h;
    float areaC = rect2.w * rect2.h;
    ...
    float areaI = (xG - xE) * (yG - yE);

    //compute the iou
    float iou = areaI / (areaA + areaC - areaI);
...
void Filter_Test()
{
    epri_filter_test filter_info = {0};
    epri_filter_test_res filter_res = {0};
    char data_base_path[] = "/mnt";
    sprintf(filter_info.command,"%s","issue_filter");
    filter_info.group_id = 1;
    strcpy(filter_info.images[0].path,"1.jpg");
    strcpy(filter_info.images[1].path,"2.jpg");
    strcpy(filter_info.images[2].path,"3.jpg");
    strcpy(filter_info.images[3].path,"4.jpg");
    strcpy(filter_info.images[4].path,"5.jpg");
    FILTER_DoTest_Task(data_base_path,&filter_info,&filter_res);
...
int FILTER_DoTest_Task(char *p_data_base_path, epri_filter_test *p_filter_info,epri_filter_test_res *p_filter_res)
{
    int i, j, ret = -1;
    char cmdname[1024] = {0};
    char command_ex[] = "issue_filter";

    epri_issues_filter issues_all[256] = {0};
    int issues_all_cnt=0;
    //epri_issues_filter issues_res[256] = {0};
    //int issues_res_cnt=0;

    int s32Ret = -1;
    //用户实际测试 函数
    SAMPLE_PRT("********** do filter test **********\n");
    ...
    //execute cpu test.
    if (!memcmp(p_filter_info->command, command_ex, strlen(command_ex)))
    {
        ...
        {
            if (strlen(p_filter_info->images[i].path) < 1)
            {
                continue;;
            }


            sprintf(cmdname, "%s/%s", p_data_base_path, p_filter_info->images[i].path);
                ...
                {                    
                     if(((int)issue_rects[ii].classId == k) && k < 8 )
                     {
                      //  printf("k %d\n",k);
                        ...
                        strcpy(issues_all[issues_all_cnt].issue_type,g_class.class_name[k]);
                        issues_all[issues_all_cnt].score = issue_rects[ii].confidence;
                        issues_all[issues_all_cnt].x = issue_rects[ii].rect.left;
                        issues_all[issues_all_cnt].y = issue_rects[ii].rect.top;
                        issues_all[issues_all_cnt].w = issue_rects[ii].rect.right-issue_rects[ii].rect.left;
                        issues_all[issues_all_cnt].h = issue_rects[ii].rect.bottom-issue_rects[ii].rect.top;
                        issues_all[issues_all_cnt].flag = 1;
                        strcpy(issues_all[issues_all_cnt].path,p_filter_info->images[i].path);
                        
                        printf("path:%s  type:%s  score:%f  %d %d %d %d  cnt%d\n",issues_all[issues_all_cnt].path,issues_all[issues_all_cnt].issue_type,issues_all[issues_all_cnt].score,
        issues_all[issues_all_cnt].x,issues_all[issues_all_cnt].y,issues_all[issues_all_cnt].w,issues_all[issues_all_cnt].h,issues_all_cnt);

                        

                        memcpy(&p_filter_res->issues[p_filter_res->issues_cnt],&issues_all[issues_all_cnt],sizeof(epri_issues_filter));
                        p_filter_res->issues_cnt++;
        ...
        all_time = (stEnd.tv_sec - stStart.tv_sec) * 1000000 + (stEnd.tv_usec - stStart.tv_usec);
        p_filter_res->inference_time = all_time;
        ...
        {            
            ...
            for(int arryidx=0;arryidx<256;arryidx++)
            {            
                for(int curr=arryidx+1;curr<256;curr++)
                {
                    if(!strcmp(p_filter_res->issues[curr].issue_type,g_class.class_name[labelidx]))
                    {
                        if(iou_filter(p_filter_res->issues[arryidx],p_filter_res->issues[curr])>0.5)
                        {
                           p_filter_res->issues[curr].flag = 0;
                           p_filter_res->issues[arryidx].flag = 0; 
                        }
                    }
                }
             }
...
/*
        ...
        for(int labelidx=0;labelidx<8;labelidx++)
        {
            ...
            for(int arryidx=0;arryidx<256;arryidx++)
            {            
                if(!strcmp(issues_all[arryidx].issue_type,g_class.class_name[labelidx]))
                {                          
                    if(isFirstFind==1)
                    {
                         memcpy(&p_filter_res->issues[p_filter_res->issues_cnt],&issues_all[arryidx],sizeof(epri_issues_filter));
                         curridx = arryidx;
                         p_filter_res->issues_cnt++;
                         isFirstFind = 0;
                         continue;
                    }
                    if(iou_filter(issues_all[curridx],issues_all[arryidx])>0.5)
                    {
                        issues_all[curridx].flag = 0;
                        issues_all[arryidx].flag = 0;
                        memcpy(&p_filter_res->issues[p_filter_res->issues_cnt],&issues_all[arryidx],sizeof(epri_issues_filter));
                        p_filter_res->issues_cnt++;
                        printf("***##@@@****%d %s  %s curridx%d arryidx%d\n",__LINE__,__FUNCTION__,issues_all[arryidx].issue_type,curridx,arryidx);
                        continue;
                    }
                    
                    issues_all[arryidx].flag = 1;
                    memcpy(&p_filter_res->issues[p_filter_res->issues_cnt],&issues_all[arryidx],sizeof(epri_issues_filter));
                    curridx = arryidx;
                    p_filter_res->issues_cnt++;
                }
...
*/
   ...
   //     char* p_data_res = cJSON_Print((cJSON*)struct_to_json_epri_filter_test_res(p_filter_res));
   //     printf("%s\n",p_data_res);

        
        
        printf("FILTER_DoTest_Task end!\n");
    }
    else
    {
        printf("command:%s err\n", p_filter_info->command);
    }
...
int filter_http_intf_init()
{
    int res = 0;

    memcpy(g_class.class_name,cl_name,sizeof(cl_name));

    // 用户注册    Json 命令回调函数        command :"issue_filter"
    res = SC_MPI_SYS_HttpServer_RegProc("issue_filter", (SC_MPI_SYS_HttpSVR_ProcJson_Cb)filter_json_proc_cb);
