...
int track_DoTest_Task(char *p_data_base_path, Ng_track_test *p_track_info,Ng_track_test_res *p_track_res);
...
static int  track_json_proc_cb(void *p_cJson)
{
    printf("track_json_proc_cb\n");
    cJSON               *p_root_cJson = (cJSON*)p_cJson;
    cJSON               *p_response_cJson = NULL;
    Ng_track_test_res    test_json_res = {0};
    Ng_track_test       *p_test_json = NULL;
    int res = 0;
    ...
    //将json 字符串转换为 结构体
    p_test_json = (Ng_track_test*)json_to_struct_Ng_track_test(p_root_cJson);
    if(NULL != p_test_json)
    {
        //获得资源的完整路径
        ...
        res = track_DoTest_Task(g_base_path, p_test_json, &test_json_res);
        /*****************************************************************************/
        ...
        strcpy(test_json_res.tracking_path, p_test_json->tracking_path);
        strcpy(test_json_res.type, p_test_json->type);
        test_json_res.group_id = p_test_json->group_id;

        //change struct to json.
        p_response_cJson = struct_to_json_Ng_track_test_res(&test_json_res);
...
void Track_Test()
{
    Ng_track_test track_info = {0};
    Ng_track_test_res track_res = {0};
    char data_base_path[] = "/mnt";
    sprintf(track_info.command,"%s","start_tracking");
    track_info.group_id = 1;
    strcpy(track_info.target_path,"data/1.jpg");
    strcpy(track_info.type,"dozer_shovel");
    track_info.roi.x = 1263;
    track_info.roi.y = 1655;
    track_info.roi.w = 150;
    track_info.roi.h = 139;
    strcpy(track_info.tracking_path,"2.jpg");
    track_DoTest_Task(data_base_path,&track_info,&track_res);
}

/* Read a file, parse, render back, etc. */
static char* dofile(char* filename)
{
    FILE* f;
    long length;
    char* data;
...
int track_DoTest_Task(char *p_data_base_path, Ng_track_test *p_track_info,Ng_track_test_res *p_track_res)
{
    int i, j, ret = -1;
    char cmdname[1024] = {0};
    char command_ex[] = "start_tracking";


    int s32Ret = -1;
    //用户实际测试 函数
    SAMPLE_PRT("********** do track test **********\n");
    ...
    //execute cpu test.
    if (!memcmp(p_track_info->command, command_ex, strlen(command_ex)))
    {
        sprintf(cmdname, "%s/%s", p_data_base_path, p_track_info->tracking_path);
        printf("image  path:%s\n", cmdname);
        ...
        Ng_track_result_data_file* pdata_file=NULL;
        int  angle_json_file;


        printf("p_track_info->target_path %s\n",p_track_info->target_path);

#if 1
        char *p = strrchr(p_track_info->target_path, '/');
        char last[50];
        if(0!=NULL)
        {
            strcpy(last, p + 1);
        ...
        {
            strcpy(last,p_track_info->target_path);
        }

        printf("last %s\n",last);


        sscanf(last, "%[^0-9]%[0-9]",img_path,json_index);
        printf("img_path%s json_index%s\n",img_path,json_index);

        sprintf(json_file,"./mnt/b/%s.json", json_index);
        printf("json_file %s\n",json_file);
#else
        if (!p_track_info->group_id)
        {
            p_track_info->group_id = 1;
        }

        sprintf(json_file,"./mnt/b/%d.json", p_track_info->group_id);
        printf("json_file %s\n",json_file);
#endif
        ...
        pdata_file = (Ng_track_result_data_file*)json_to_struct_Ng_track_result_data_file(json_root);
        printf(">>>>>>pdata_file->result.angle %d\n",pdata_file->result.angle);

        srand(time(0));
        float b=rand()/(RAND_MAX+1.0);//0-1随机数
        angle_json_file = ((int)((pdata_file->result.angle + pdata_file->result.angle*0.1*(-1+2*b))*1000))/1000.0;
        printf(">>>>>>angle_res %d\n",angle_json_file);
        ...
        {
           if(!strcmp(p_track_info->type,g_class.class_name[issue_rects[ii].classId]))
           {
               p_track_res->roi.x = issue_rects[ii].rect.left;
               p_track_res->roi.y = issue_rects[ii].rect.top;
               p_track_res->roi.w = issue_rects[ii].rect.right-issue_rects[ii].rect.left;
               p_track_res->roi.h = issue_rects[ii].rect.bottom-issue_rects[ii].rect.top;

               p_track_res->offset_pos.x = p_track_res->roi.x - p_track_info->roi.x;
               p_track_res->offset_pos.y = p_track_res->roi.y - p_track_info->roi.y;
               p_track_res->offset_pos.w = p_track_res->roi.w - p_track_info->roi.w;
               p_track_res->offset_pos.h = p_track_res->roi.h - p_track_info->roi.h;

               p_track_res->angle = angle_json_file;
           }
        }


        //char* p_data_res = cJSON_Print((cJSON*)struct_to_json_Ng_track_test_res(p_track_res));
        //printf("%s\n",p_data_res);


        printf("track_DoTest_Task end!\n");
    }
    else
    {
        printf("command:%s err\n", p_track_info->command);
    }
...
int track_http_intf_init()
{
    int res = 0;

    memcpy(g_class.class_name,cl_name,sizeof(cl_name));

    // 用户注册    Json 命令回调函数        command :"start_tracking"
    res = SC_MPI_SYS_HttpServer_RegProc("start_tracking", (SC_MPI_SYS_HttpSVR_ProcJson_Cb)track_json_proc_cb);
